...
class TestRemoveRedundantSmarts:
    ...
    def test_keeps_parameters_without_ids(self):
        """Test that parameters without an ID are kept rather than raising.

        The ID is optional in the SMIRNOFF spec, and force fields using a
        double-exponential functional form omit it. Such parameters cannot have
        been generated by PRESTO, so they are never removal candidates.
        """
        mol = openff.toolkit.Molecule.from_smiles("CCO")  # Ethanol
        ff = openff.toolkit.ForceField(
            "openff_unconstrained-2.3.0.offxml", load_plugins=True
        )

        # Swap the vdW handler for a double-exponential one, whose parameters
        # carry no IDs.
        ff.deregister_parameter_handler("vdW")
        de_handler = ff.get_parameter_handler(
            "DoubleExponential",
            {"version": "0.3", "alpha": 18.7, "beta": 3.3, "scale14": 0.5},
        )
        for smirks, r_min, epsilon in [
            ("[#1:1]", 1.2, 0.06),
            ("[#6:1]", 1.9, 0.1),
            ("[#8:1]", 1.7, 0.2),
        ]:
            de_handler.add_parameter(
                {
                    "smirks": smirks,
        ...
        assert all(p.id is None for p in de_handler.parameters)

        # Add an unused bespoke parameter to confirm pruning still happens
        # alongside the parameters without IDs.
        bond_handler = ff.get_parameter_handler("Bonds")
        _add_parameter_with_overwrite(
            bond_handler,
            {
                "smirks": "[#99:1]-[#99:2]",
                "id": "b-bespoke-unused",
                "length": 1.2 * unit.angstrom,
                "k": 600.0 * unit.kilocalorie_per_mole / unit.angstrom**2,
            },
        )

        ff_cleaned = _remove_redundant_smarts(mol, ff, id_substring="bespoke")

        # The parameters without IDs are untouched
        cleaned_de = ff_cleaned.get_parameter_handler("DoubleExponential").parameters
        assert len(cleaned_de) == 3
        assert all(p.id is None for p in cleaned_de)

        # ... and the unused bespoke parameter is still removed
        cleaned_bond_ids = [
            p.id for p in ff_cleaned.get_parameter_handler("Bonds").parameters
        ]
        assert "b-bespoke-unused" not in cleaned_bond_ids
